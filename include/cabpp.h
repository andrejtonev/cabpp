...
class CABpp {
 public:
   ...
   *        If this is not acceptable for a specific object, objects can be
   * created by the user, with their pointers being passed to the CABpp
   * constructor.
   ...
   *        need to be constant so every object is constructed in the same way.
   *        If this is not acceptable for a specific object, objects can be
   * created by the user, whith their pointers being passed to the CABpp
   * constructor.
    ...
    for (int i = 0; i < slots; ++i) {
      ...
      if (std::align(alignof(T), sizeof(T), ptr, mem_size)) {
        ...
        std::cerr << "Failed while aligning memory"
                  << ((mem_size < sizeof(T)) ? ": out of memory" : "")
                  << std::endl;
        return;
      }
    }
  ...
  ObjPtr<T> Reserve(void) {
    ...
    if (idx >= ptr_flags_.size()) return ObjPtr<T>(nullptr, -1);
  ...
  bool Write(ObjPtr<T>& obj_ptr) {
    if (!obj_ptr || obj_ptr.ptr_idx_ >= ptrs_.size() ||
        obj_ptr.obj_ptr_ != ptrs_[obj_ptr.ptr_idx_])
      return false;
    // Update read share pointer
    std::atomic_store(&read_sp_,
                      CreateSharedPtr(obj_ptr.obj_ptr_, obj_ptr.ptr_idx_));
    // Reset ObjPtr
   ...
   */
  std::shared_ptr<const T> Read() const { return std::atomic_load(&read_sp_); }
 ...
 private:
  ...
  void DeleteSP(T* elem, int idx) {
    if (idx >= ptr_flags_.size())
      std::cerr << "Failed to set the pointer (idx " << idx << ") to writable"
                << std::endl;
    else
  ...
  std::shared_ptr<const T> CreateSharedPtr(T* ptr, int idx) {
    ...
    return std::shared_ptr<const T>(
        ptr, std::bind(&CABpp<T>::DeleteSP, this, _1, idx));
  }
  ...
  class Flag {
   public:
    ...
    Flag(const bool flag_in) { flag = flag_in; }

    Flag(const Flag& in) { flag = in.flag.load(); }
   ...
   */
  std::vector<Flag>
      ptr_flags_;         //!< Read/Write flags vector (access type definition)
  std::vector<T*> ptrs_;  //!< Object pointer vector
  std::shared_ptr<const T> read_sp_;  //!< Class' shared pointer passed on
                                      //!< reading (atomic operations)
...
class ObjPtr {
 public:
  ...
  operator bool() const { return (obj_ptr_ != nullptr || ptr_idx_ >= 0); }
